...
Failed use case functions will result in an indication of failure a `Result` object construct. Any violation of the module use case function API will result in either a a `panic!` when it comes to `codemelted.rs` or thrown `SyntaxError` for `codemelted.js` indicating what was violated.
